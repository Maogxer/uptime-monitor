...
<template>
  <div>
    ...
    <div v-else-if="error" class="text-center py-10 bg-white/50 dark:bg-white/5 backdrop-blur-xl rounded-2xl shadow-lg"><p class="text-xl font-bold text-red-500">{{ t('details.errorTitle') }}</p><p class="text-sm text-gray-500 mt-2">{{ error.statusMessage }}</p></div>
        ...
        <div v-for="group in monitorGroups" :key="group.key" v-show="group.monitors.length > 0">
          ...
          <div class="flex items-center justify-between mb-4">
            ...
            <button 
              ...
              :title="sortOrder === 'desc' ? t('tooltips.sortDesc') : t('tooltips.sortAsc')"
            >
...
const monitorGroups = computed(() => {
    ...
    return [
        { key: 'down', title: t('groups.down'), monitors: groups.down, borderColor: 'border-red-500', sortable: false },
        { key: 'up', title: t('groups.up'), monitors: groups.up, borderColor: 'border-green-500', sortable: true },
        { key: 'paused', title: t('groups.paused'), monitors: groups.paused, borderColor: 'border-gray-400', sortable: false },
    ];
});
</script>